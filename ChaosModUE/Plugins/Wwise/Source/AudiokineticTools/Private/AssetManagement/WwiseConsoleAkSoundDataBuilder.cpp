...
void WwiseConsoleAkSoundDataBuilder::ProcessFile(const FString& FullPath, const FString& GeneratedSoundBanksFolder)
{
	...
	if (AkAudioEventIt)
	{
		...
		if (bIsBank)
		{
			...
			AllReadTasks.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, FullPath, Platform, RequiredData]
			{
		...
		{
			FScopeLock AutoLock(&ReadTaskLock);
			AllReadTasks.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, FullPath, Platform, Language, RequiredData]
			{
				if (RequiredData->AkAudioEvent && RequiredData->EventPlatformData)
				{
	...
	else if (AkAuxBusIt)
	{
		...
		if (AkAuxBus)
		{
			...
			if (AuxBusPlatformData)
			{
				...
				{
					FScopeLock AutoLock(&ReadTaskLock);
					AllReadTasks.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, AkAuxBus, AuxBusPlatformData, FullPath] {
				...
				{
					FScopeLock AutoLock(&ReadTaskLock);
					AllReadTasks.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, AkAuxBus, AuxBusPlatformData, Platform, FullPath] 
					{
	...
	else if (FileName == AkInitBankName || FileName == AkPluginInfoFileName)
	{
		...
		{
			FScopeLock AutoLock(&ReadTaskLock);
			AllReadTasks.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, InitBank, InitBankPlatformData, FullPath] 
			{
		...
		{
			FScopeLock AutoLock(&ReadTaskLock);
			AllReadTasks.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, InitBank, Platform, FullPath] 
			{
		...
		{
			FScopeLock AutoLock(&ReadTaskLock);
			AllReadTasks.Add(FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, InitBank, InitBankPlatformData, Platform, FullPath]
			{
